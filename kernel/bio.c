...
struct
{
  struct spinlock lock; // protect buf
  struct buf buf[NBUF];

  // Hash map: dev and blockno to buf
  struct buf bufmap[NBUCKET];
  struct spinlock bufmap_locks[NBUCKET]; // protect each bucket
...
void binit(void)
{
  ...
  for (int i = 0; i < NBUF; ++i)
  {
    ...
    b->lastuse = 0;
    b->refcnt = 0;
    // when init ,all buf is in first bucket
...
bget(uint dev, uint blockno)
{
  ...
  uint key = BUFMAP_HASH(dev, blockno); // calc hash key
  ...
  for (b = bcache.bufmap[key].next; b; b = b->next)
  {
    ...
    { // find ,return
      ...
      acquiresleep(&b->lock); // confirm no used by other proc
      return b;
    }
  }
  release(&bcache.bufmap_locks[key]); // in case of deadlock
